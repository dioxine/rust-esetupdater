...
Rename config.toml.example to config.toml, read comments in it, edit, change ```host``` to the actual server you have access and you are ready to go! Also supports command line directives so you can use program without config file. Only ```host``` and ```root_dir``` parameters are accessible via command line. ```user-agent``` in that case will be used built-it.
